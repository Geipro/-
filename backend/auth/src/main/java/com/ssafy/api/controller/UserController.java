...
public class UserController {
	...
	public ResponseEntity<UserRes> getMyInfo(@ApiIgnore Authentication authentication) {
		 ...
		 * 식별. 액세스 토큰이 없이 요청하는 경우, 403 에러({"error": "Forbidden", "message": "AccessDenied"}) 발생.
		 */
...
//		User user = userService.getUserByEmail(email);
		Profile profile = profileService.getUserByUserId(userId);
	...
	public ResponseEntity<Boolean> modifyProfile(@RequestBody @ApiParam(value="프로필 정보수정", required = true) UserModifyInfoReq modifyInfo, @ApiIgnore Authentication authentication){
		SsafyUserDetails userDetails = (SsafyUserDetails) authentication.getDetails();
		String userId = userDetails.getUserId();
